...
import { OrientationPicker, getOrientationLabel, LABEL_ORIENTATIONS } from '@/components/labels/OrientationPicker';

interface OrientationConfirmBannerProps {
  orientation: number;
  confirmed: boolean;
  onConfirm: (selectedOrientation: number) => Promise<void>;
  isPending?: boolean;
}
...
export function OrientationConfirmBanner({ orientation, confirmed, onConfirm, isPending }: OrientationConfirmBannerProps) {
  ...
  const orientationData = LABEL_ORIENTATIONS.find(o => o.number === (confirmed ? orientation : selectedOrientation));
  const label = getOrientationLabel(confirmed ? orientation : selectedOrientation);

  const handleConfirm = async () => {
    setConfirming(true);
    try {
      await onConfirm(selectedOrientation);
    } finally {
      setConfirming(false);
    }
  };
  ...
  return (
    <div className={`rounded-2xl border-2 p-5 transition-colors ${
      ...
      <div className="space-y-4">
        ...
        {/* Orientation Picker */}
        <OrientationPicker
          value={confirmed ? orientation : selectedOrientation}
          onChange={confirmed ? undefined : setSelectedOrientation}
          readOnly={confirmed}
        />

        {/* Current selection label */}
        <p className="text-sm font-medium text-slate-700 text-center sm:text-left">{label}</p>
        {orientationData && (
