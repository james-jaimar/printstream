...
export function LabelItemsDropZone({
  orderId,
  dieline,
  onFilesUploaded,
  disabled = false,
}: LabelItemsDropZoneProps) {
  ...
  const processFiles = async (files: File[]) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const filePath = `label-artwork/orders/${orderId}/${fileName}`;
            ...
            {dieline && (
              <div className="flex items-center gap-2 text-xs text-muted-foreground bg-muted px-3 py-1.5 rounded-full">
                ...
                <span>
                  ...
                  {dieline.label_height_mm + (dieline.bleed_top_mm ?? 1.5) + (dieline.bleed_bottom_mm ?? 1.5)}mm with bleed
                </span>
              </div>
            )}
          </>
